                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                                ...
                                {selectedEntity.sdfCollider && (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '8px', background: '#000', borderRadius: 4 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <span>Width / R1</span>
                                            <input type="number" min="0.1" max="100" step="0.1" value={selectedEntity.sdfCollider.size.x} onChange={e => { 
                                                selectedEntity.sdfCollider!.size.x = Number(e.target.value); 
                                                setSelectedEntity({...selectedEntity}); 
                                            }} style={{ width: 60, background: '#111', color: '#fff' }}/>
                                        </div>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <span>Height / R2</span>
                                            <input type="number" min="0.1" max="100" step="0.1" value={selectedEntity.sdfCollider.size.y} onChange={e => { 
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        ...
                                        <input type="number" min="0.1" max="1000" step="0.1" value={selectedEntity.physics.mass} onChange={e => { 
                                            ...
                                            if (selectedEntity.physics!.particleIdx !== undefined) {
                                                engineRef.current?.physics.setParticleInvMass(selectedEntity.physics.particleIdx, 1.0 / m);
